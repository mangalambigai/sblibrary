    ...
    var tableToCSV = function(table) {
        ...
        for (i = 0; i < table.rows.length; i++) {
            ...
            for (j = 0; j < row.cells.length; j++) {
                col = row.cells[j];
                console.log(col);
                ///EDITED: don't get visible:xs elements
                //data = data + (j ? csvDelimiter : '') + fixCSVField(col.textContent.trim());
                var textcontent = '';
                var cellChildren = col.childNodes;
                var k;
                for ( k = 0; k < cellChildren.length; k++) {
                    if (cellChildren[k].nodeType === Node.ELEMENT_NODE)
                    {
                        if (cellChildren[k].classList.contains("visible-xs"))
                        {
                            continue;
                        }
                        if (cellChildren[k].tagName==='BUTTON'||
                            cellChildren[k].tagName === 'SPAN')
                            continue;
                    }
                    textcontent += cellChildren[k].textContent.trim();
                }
                data = data + (j ? csvDelimiter : '') + fixCSVField(textcontent);
            }
